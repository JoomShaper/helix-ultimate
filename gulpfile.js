...
const config = {
    ...
    packageName: 'Helix_Ultimate_v2.0.0.zip',
    ...
    toArray: function(key) {
        ...
        else if (typeof this[key] === 'function')   return this[key]().split(',').map(v => v.trim());
    },
...
function pluginStreamTask() {
    return src([
        path.resolve(config.srcPath, './plugins/system/helixultimate') + '/**/*.{' + config.parseExtensions('pluginFileExtensions') + '}'
