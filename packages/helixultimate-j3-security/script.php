...
class Helixultimatej3securityfixesInstallerScript
{
	...
	public function preflight($type, $parent)
	{
		...
		if (version_compare($helixVersion, self::SUPPORTED_HELIX_MIN, '<')
			|| version_compare($helixVersion, self::SUPPORTED_HELIX_MAX, '>'))
		{
			Factory::getApplication()->enqueueMessage(
				sprintf(
					'Unsupported Helix Ultimate version %s. Supported range: %s to %s.',
					$helixVersion,
					self::SUPPORTED_HELIX_MIN,
