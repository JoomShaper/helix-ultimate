...
<nav class="mod-breadcrumbs__wrapper" aria-label="<?php echo htmlspecialchars($module->title, ENT_QUOTES, 'UTF-8'); ?>">
	...
	if ($itemsCounter) {
		...
		$json = json_encode($data, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | $prettyPrint);

		if (version_compare(JVERSION, '4.0', '>=')) {
			// Joomla 4+
			/** @var WebAssetManager $wa */
			$wa = $app->getDocument()->getWebAssetManager();
			$wa->addInline('script', $json, [], ['type' => 'application/ld+json']);
		} else {
			// Joomla 3 fallback using addCustomTag method
