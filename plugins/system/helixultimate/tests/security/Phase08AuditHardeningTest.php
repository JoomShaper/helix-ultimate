 ...
 * - HU-7B-C1 (selected-menu authority boundary)
 * - HU-5A3-1 (trusted update-site association & extra_query preservation)
 * - HU-4B-01 (exact media operation authority)
 * - HU-4B-02 (fail-closed image MIME & full decode check)
 * - HU-7A-01 (article object-level authority & ownership)
 * - HU-8A2-C4b (quoted and escaped CSS background URL)
 * - V2210-CAND-UPGRADE-01 (vendor cleanup)
 * - V2210-CAND-DEP-01 (SCSSPHP dependency coherence)
 *
...
final class Phase08AuditHardeningTest
{
	public static function run(): array
	{
		...
		// 2. Test composer.json alignment
		$composerJsonPath = dirname(__DIR__, 2) . '/composer.json';
		if (is_file($composerJsonPath))
		{
			$composer = json_decode((string) file_get_contents($composerJsonPath), true);
			$scssReq = $composer['require']['scssphp/scssphp'] ?? '';

			if (!str_contains($scssReq, '^2.'))
			{
				$failures[] = "composer.json scssphp/scssphp requirement ({$scssReq}) should require ^2.x for PHP 8.x compatibility";
			}
		}

		// 3. Verify media permissions map
